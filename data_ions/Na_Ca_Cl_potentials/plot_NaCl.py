...
pmf_pn_target=np.loadtxt("pmf_NaCl.xvg",unpack=True)
pmf_pn_GT=np.loadtxt("pmf_NaCl_GT.xvg",unpack=True)

VR1pnEwald=np.loadtxt("VR1NaCl_Ewald.txt",unpack=True)
...
plt.figure()
ax1 = plt.subplot(111)
ax1.plot(r,np.exp(-(pmf_target(r)-pmf_target(1.25))),color="black",lw=3,label=r"$g_{\mathrm{NaCl}}(r) $")
ax1.plot(r,np.exp(-(pmf_GT(r)-pmf_GT(1.25)+(VR1(r)-VR1(1.25))/kT*138.9)),color='red',ls='--',lw=3,dashes=(8, 8),label=r"$g_{\mathrm{R},\mathrm{NaCl}}(r) $")
ax1.plot(r,np.exp(-(pmf_GT(r)-pmf_GT(1.25))),color='green',ls='-.',lw=3,dashes=(2, 2),label=r"$g_{0,\mathrm{NaCl}}(r) $")
